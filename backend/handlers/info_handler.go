...
func getAccountInfo(c *gin.Context) {
	...
	if role == "admin" {
		...
		if err != nil {
			fmt.Println("err: ", err)
			sendData(c, http.StatusInternalServerError, gin.H{"err": err}, "区块链执行异常")
			return
		}
...
func getMyTx(c *gin.Context) {
	...
	if err != nil {
		sendData(c, http.StatusForbidden, gin.H{"err": err}, "获取自己相关交易失败")
		return
	}
...
func getBillFromMe(c *gin.Context) {
	...
	if err != nil {
		fmt.Println("err: ", err)
		sendData(c, http.StatusForbidden, gin.H{"err": err}, "获取应付账单失败")
		return
	}
...
func getBillToMe(c *gin.Context) {
	...
	if err != nil {
		fmt.Println("err: ", err)
		sendData(c, http.StatusForbidden, gin.H{"err": err}, "获取应收账单失败")
		return
	}
